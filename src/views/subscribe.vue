...
<div class="container">
  ...
  <div class="block">
    ...
    <div class="content" style="margin-left:-1rem;margin-top:-1rem;">
      <div class="scroll-view">
        <div class="scroll-item" v-for="(item,index) in recycleClass" :key="index" @click="recycleChange(index)">
          <div :class="{'scroll-item-img':true, 'scroll-item-img-selected':currentRecycle === index}">
            <van-image fit="contain" :src="item.img"  with="100" hight="100"></van-image>
          </div>
          <div>{{item.title}}</div>
        </div>
      </div>
  ...
  <div class="block"  style="padding:0 0.5rem">
    <div class="content">
      ...
      <div class="area_unavailable_tip" v-else style="margin-top:1.5rem">当前区域未开通</div>
    </div>
  </div>
      <van-divider></van-divider>
  ...
  <div class="block" style="padding:0 1rem 0 1rem">
    ...
    <div class="content">
      <!-- 重量范围 -->
      <div class="grid">
        <WeightRange class="item" @click.native="selectWeight(index)" v-for="(item,index) in estimateWeight" :key="index" :checked="currentEstimateWeight === index">
          {{ item.range }}
        </WeightRange>
      </div>

      <div class="item">
        <van-icon name="warning" color="#FFA555" size="19px" class="item_icon"></van-icon>
  ...
  <div class="block" style="padding:0 1rem 0 1rem">
    <div class="content">
      <div class="item">
        <van-icon name="clock" color="#C0C0C0" size="19px" class="item_icon"></van-icon>
        <div class="title">上门时间</div>
        <div class="flex_title">
          <div :class="{content:true ,active: servicetime != ''}" @click="timePickerShow = true;">{{servicetime != '' ? formattime : '请选择上门时间'}}</div>
          <div v-if="timePickerShow" style="position:fixed;left:0;top:0;width:100vw;height:100vh; background-color:rgba(0,0,0,0.3);z-index:11;">
            <van-datetime-picker
      ...
      <!-- <van-divider></van-divider> -->
      <div class="item">
        <van-icon name="location" color="#C0C0C0" size="19px" class="item_icon"></van-icon>
        <div class="title">预约地址</div>
        <div class="flex_title">
          <div :class="{content:true ,active: currentAddress != ''}" @click="changeAddress">{{currentAddress != '' ? currentAddress : '请选择预约地址'}}</div>
          <van-icon name="arrow" color="#c0c0c0" size="16px"></van-icon>
        </div>
      </div>
    </div>
  ...
  <div class="footer">
    <div class="clause">预约下单代表您已同意<router-link to="/disclaimers" class="link">《免责条款》</router-link></div>
    <van-button :type="allowSubmit ? 'primary' : 'default'" round size="large" :disabled="!allowSubmit" style="height:50px">
...
<script>
  ...
  export default {
    ...
    mounted(){
      this.now = new Date();
      if(this.currentAddr){} //不要删除这行，会出bug
    },
    methods:{
        initMap(){
          AMapLoader.load({
        ...
        recycleChange(id) { //设置选中的回收分类
          this.currentRecycle = id
        },
        timePickerChange(e){ //切换时间选择器打开状态
          this.timePickerShow = false
        },
        filter(type, options){ //时间选择器时间范围过滤
          let now = new Date()
          if(type === 'year'){
            return options.filter(option => option < now.getFullYear() + 1)
          }else if(type === 'hour'){
            return options.filter(option => Number.parseInt(option) >= 8 && Number.parseInt(option) <= 20)
          }else if(type === 'day'){
            let day = now.getDate()
            let hour = now.getHours()
            return options.filter(option => (hour >= 20 && option != day) || hour < 20)
          }
          return options
        },
        selectWeight(id){ //选择重量范围
          this.currentEstimateWeight = id
        },
        changeAddress(){ //点击修改地址按钮
          this.$router.push('/recyclingadd')
        }
    },
    watch:{
      async currentAddress(val){ //异步方法，地名转经纬度
        try{
            let res = await this.axios.get('https://restapi.amap.com/v3/geocode/geo?parameters',{
            params:{
      ...
      currentGeo(val){ //通过监听经纬度数组的长度，切换“当前是否在可用区域内”
        if(val.length === 2){
    ...
    computed:{
      formattime(){ //格式化日期，仅用于显示，实际提交表单应提交 this.servicetime
        let time = new Date(this.servicetime)
        return time.getFullYear() + '年' + (time.getMonth() + 1) + '月' + time.getDate() + '日 ' + time.getHours() + ':00'
      },
      currentAddr(){
        if(this.$store.state.address.currentSelectId === '-1'){
          return ''
        }else{
          let temp = this.$store.state.address.address[Number.parseInt(this.$store.state.address.currentSelectId)]
...
.grid > .item{
  ...
  width: calc(50% - .2rem);//使负外边距移动自身距离高与宽的一半
  box-sizing: border-box;
